...
class GFlowNetAgent:
    def __init__(
        ...
        **kwargs,
        ...
        # Environment
        self.env = env#这里的env指具体的环境类，如aptamers
        self.mask_source = self._tbool([self.env.get_mask_invalid_actions_forward()])#这里的_tbool是将bool类型的数组转换为tensor类型；
        #这里的get_mask_invalid_actions_forward()是指env/sequence类中的获取mask动作列表的方法
        # Continuous environments这块可以舍去
        self.continuous = hasattr(self.env, "continuous") and self.env.continuous#如果env中有continuous属性且为True，则为连续环境
        ...
        # Policy models（用于输出不同动作的概率）
        #前向传播过程
        self.forward_policy = Policy(policy.forward, self.env, self.device, self.float)#Policy类在下面定义
        ...
        # Optimizer
        if self.forward_policy.is_model:#如果forward_policy是模型（采用TB为目标函数）
    ...
    def sample_actions(
        ...
        ----
        envs最为重要，需要确认
        env.state代表一个状态节点（可以理解为一个中间状态，数组序列[0,1,2,0]，states即[state1,state2,...]
        ...
        #state:单个轨迹的状态表示（列表形式）
        if is_forward:#判断当前是否是前向传播
            mask_invalid_actions = self._tbool(
                [env.get_mask_invalid_actions_forward() for env in envs]#
            )#对于当前状态，获取该状态下的无效动作,最终获得所有状态的无效动作列表
        else:
            mask_invalid_actions = self._tbool(
                [env.get_mask_invalid_actions_backward() for env in envs]
            )
        # Build policy outputs构建policy的输出
        ...
        idx_norandom = (
            ...
            .sample()
            .to(bool)
        )#这段代码的意思是，对于每个state，以1-random_action_prob的概率采样一个bool值，如果为True，则对应的state不采用随机动作
        
        # Check for at least one non-random action#检查是否至少有一个非随机动作
        ...
        # Sample actions from policy outputs基于policy的输出采样动作（policy输出不同动作的logits，采样方法为policy，mask_invalid_actions为当前state下的
        # 无效动作，temperature为1.0，用于对logits进行调整），最终返回有效的动作列表
        actions, logprobs = self.env.sample_actions(
            policy_outputs,#states，包含一条轨迹的所有中间状态，对每个中间state都会输出一个logits
            sampling_method,
            mask_invalid_actions,#所有状态的无效动作列表的列表集合
            temperature,
        )
        return actions
    ...
    def sample_batch(
        ...
        if train == True:#如果这批数据用于训练
            Each item in the batch is a list of 7 elements (all tensors):每个轨迹（列表形式）包含以下7个元素
                - [0] the state
                - [1] the action
        ...
        def _add_to_batch(batch, envs, actions, valids, train=True):
            for env, action, valid in zip(envs, actions, valids):
                if valid is False:#如果当前动作无效，则跳过
                    continue
                parents, parents_a = env.get_parents(action=action)
                ...
                if train:#如果是训练，则将轨迹中的每个中间状态都加入到batch中
                    batch.append(
                        [
                ...
                else:#如果是采样，则将轨迹中的最后一个状态加入到batch中
                    if env.done:
        ...
        t0_all = time.time()
        batch = []
        if isinstance(envs, list):#如果是列表，则对列表中的每个env进行操作
    ...
    #计算一个batch的trajectory balance loss
