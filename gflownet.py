...
import json
with gzip.GzipFile('data/smiles_list.json.gz','rb') as f_in:
    smiles_list = json.load(f_in.read().decode())
...
class GFlowNetAgent:
    def __init__(
        ...
        **kwargs,
        ...
        self.forward_policy_2 = Policy(policy.forward_2, self.env, self.device, self.float)
        #todo:set_forward_policy_ckpt_path修改
...
class Policy:
    def __init__(
        ...
        self.data = pd.read_csv(count_path_1)
        self.count = dict(zip(self.data["SMILES_index"], self.data["COUNT"]))
        self.tree = HuffmanTree(self.count)
        ...
        if self.type == "fixed_1":
            ...
            self.model = h_softmax(vocabulary_size = len(self.vocab_1),
                                    embedding_dim = self.embedding_dim_1,
                                    tree_size = len(self.tree_1)
                                    )
            self.is_model = True
        else:
